  ...
  ({ label, error, helperText, className = '', ...props }, ref) => {
    return (
      <div className="w-full flex flex-col mb-4">
        ...
        <textarea
          ref={ref}
          className={`bg-gray-charcoal text-white/90 font-dm text-base px-4 py-3 rounded-[4px] border placeholder:text-white/40 transition-all duration-200 ease-out min-h-[120px] resize-y focus:outline-none focus:border-gold focus:shadow-[0_0_8px_rgba(212,175,55,0.3)] ${error ? 'border-error' : props.value ? 'border-success' : 'border-gray-dark'} ${className}`}
          {...props}
        />
